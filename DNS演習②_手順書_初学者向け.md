...
```
### Q. 演習②で `www IN A <WebサーバのパブリックIP>`、`db IN A <DBサーバのプライベートIP>` となっているのはなぜですか

### A.
**その名前を使う相手が違うから** です。

- `www`  
  → インターネット上の利用者がブラウザでアクセスする名前
- `db`  
  → WordPress など内部のサーバが、DB接続のために使う名前
